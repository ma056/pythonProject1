...
def main(taskid):
    ...
    i = 0
    # select hitid from tasks_hits where taskid
    ...
    for key, value in mhitid_dict.items():
        ...
        # 当3834一人做两条数据的数据符合下面判断则写入数据库，else后则是不写入
        if hit_data_dict_1.get('q_one') != hit_data_dict_2.get('q_one') or \
                hit_data_dict_1.get('q_two') != hit_data_dict_2.get('q_two') or \
                hit_data_dict_1.get('q_three') != hit_data_dict_2.get('q_three') or \
                hit_data_dict_1.get('q_four') != hit_data_dict_2.get('q_four') or \
                hit_data_dict_1.get('q_five') != hit_data_dict_2.get('q_five'):
            # 拿到3844的数据进行处理 select data_handled, done_by from tasks_hits where hitid
